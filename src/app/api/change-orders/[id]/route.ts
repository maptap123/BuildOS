...
export async function PATCH(
  ...
  const allowed = [
    'title', 'description', 'status', 'type', 'amount', 'reason',
    'submitted_date', 'approved_date', 'approved_by', 'budget_line_id',
  ]
  ...
  // When approving, stamp approved_by with current user if not provided
  if (updates.status === 'approved' && !updates.approved_by) {
    updates.approved_by = user.id
    updates.approved_date = updates.approved_date ?? new Date().toISOString().split('T')[0]
  }
